...
export default function ResultPanel({ result }: { result: { engineOutput: EngineOutput; narrative: string } }) {
    ...
    <div className="space-y-6">
      <div className="section-card">
        <div className="mb-3 flex flex-wrap items-center gap-3">
          <Badge label={engineOutput.classification} />
          <Badge label={engineOutput.risk.label} />
          <span className="text-xs text-ink/50">Risk tier</span>
        </div>
        <List title="Classification rationale" items={engineOutput.classificationRationale} />
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
        ...
        <div className="section-card"><List title="Immediate management" items={engineOutput.immediateManagement} /></div>
        <div className="section-card"><List title="Antithrombotic therapy" items={engineOutput.antithrombotic} /></div>
        <div className="section-card"><List title="Reperfusion / invasive strategy" items={engineOutput.reperfusionOrInvasiveStrategy} /></div>
        <div className="section-card"><List title="Secondary prevention (GDMT)" items={engineOutput.secondaryPrevention} /></div>
        <div className="section-card"><List title="Rehabilitation" items={engineOutput.rehabilitation} /></div>
        <div className="section-card"><List title="Short-term (inpatient) plan" items={engineOutput.shortTermPlan} /></div>
